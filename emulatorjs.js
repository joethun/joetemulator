    ...
    function switchTheme(theme) {
      ...
      popups.forEach((popup) => {
        popup.style.transition = "background-color 0.3s, border-color 0.3s";
        popup.style.backgroundColor = selectedTheme.boxBackgroundColor;
        popup.style.borderColor = selectedTheme.boxBorderColor;
        popup.style.color = "#f9f9f9";
      });

      const themeColor = selectedTheme.textColor;

      const elements = document.querySelectorAll('[data-theme-color="true"]');
      elements.forEach((element) => {
        element.style.transition = "color 0.3s";
        element.style.color = selectedTheme.textColor;
      });

      const linkElements = document.querySelectorAll("a");
      linkElements.forEach((element) => {
        element.style.transition = "text-decoration-color 0.3s";
        element.style.textDecorationColor = themeColor;
      });
    }
