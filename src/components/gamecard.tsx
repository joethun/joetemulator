...
export const GameCard = memo(({
  ...
  const handleAction = (action: (g: Game) => void) => (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
  ...
  return (
    ...
    >
      {isDeleteMode && !isUploading && (
        <>
          <div className="absolute inset-0 z-2 flex items-center justify-center rounded-xl pointer-events-none transition-opacity duration-300 bg-red-500/85" style={{ opacity: isSelected ? 1 : 0 }}>
      ...
      {isUploading && (
        <div className={`absolute inset-0 z-3 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm transition-all duration-500 ease-out ${game.isComplete ? 'opacity-0' : 'opacity-100'}`}>
          <div className="relative w-24 h-24">
            <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="40" fill="none" stroke={colors.midDark} strokeWidth="10" className="opacity-80" />
              <circle cx="50" cy="50" r="40" fill="none" stroke={colors.highlight} strokeWidth="10" strokeDasharray="251.2" strokeDashoffset={251.2 * (1 - (game.progress || 0) / 100)} strokeLinecap="round" className="transition-all duration-200 ease-linear" />
            </svg>
      ...
      <div className="h-full flex items-center justify-center bg-cover bg-center bg-no-repeat relative" style={coverStyle}>
        {!game.coverArt && (
          <div className="flex items-center justify-center w-full px-2 sm:px-4" style={{ color: colors.darkBg }}>
            <span className="block w-full max-w-full text-lg sm:text-xl md:text-2xl font-bold tracking-wide text-center break-words whitespace-normal select-none line-clamp-4" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.2)', hyphens: 'auto' }}>
              {game.title}
            </span>
            {onCoverArtClick && !isDeleteMode && !isUploading && (
              <button onClick={handleAction(onCoverArtClick)} className="absolute inset-0"><span className="sr-only">Add cover art</span></button>
            )}
          </div>
        )}
      </div>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent backdrop-brightness-75 flex flex-col justify-end p-3 sm:p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-400 ease-out">
            ...
            <div className="grid gap-2 items-stretch mt-2 sm:mt-3" style={{ gridTemplateColumns: '1fr 44px 44px' }}>
              <button className="flex items-center justify-center gap-1.5 sm:gap-2 font-bold py-2 sm:py-2.5 px-2 sm:px-4 rounded-xl transition-all text-xs sm:text-sm hover:shadow-md active:scale-[0.98] w-full" style={{ ...getGradientStyle(colors.gradientFrom, colors.gradientTo), color: colors.darkBg }} onClick={handleAction(onPlay)}>
                <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" /> PLAY
              </button>
              <button className="rounded-xl transition-all hover:shadow-md active:scale-95 flex items-center justify-center w-full h-full" style={{ backgroundColor: colors.highlight, color: colors.darkBg }} onClick={handleAction(onEdit)}>
                <Settings className="w-4 h-4 sm:w-5 sm:h-5" />
              </button>
              <button className="rounded-xl transition-all hover:shadow-md active:scale-95 flex items-center justify-center select-none bg-red-500 text-white w-full h-full" onMouseDown={startLongPress} onTouchStart={startLongPress} onMouseUp={clearLongPress} onMouseLeave={clearLongPress} onTouchEnd={clearLongPress} onClick={handleAction(onDelete)}>
