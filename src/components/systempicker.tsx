...
interface SystemListSectionProps {
    colors: ThemeColors;
    categories: Record<string, Array<[string, string]>>;
    searchQuery: string;
    onSearchChange: (q: string) => void;
    currentCore: string | null | undefined;
    onSelectSystem: (core: string) => void;
    ...
    return (
        ...
        >
            <div
                ...
                className="p-6 rounded-xl max-w-7xl w-full max-h-[90vh] flex flex-col shadow-2xl border overflow-hidden focus:outline-none"
                style={{
...
const CoverArtSection = memo(({ colors, gradient, coverArtState }: CoverArtSectionProps) => {
    ...
    const handleFitToggle = useCallback(() => {
        coverArtState.onFitChange(coverArtState.fit === 'contain' ? 'cover' : 'contain');
    }, [coverArtState]);
            ...
            <div className="p-4 sm:p-6 rounded-xl border-[0.125rem] flex flex-col" style={{ backgroundColor: colors.darkBg, borderColor: colors.midDark, boxShadow: '0 4px 12px rgba(0, 0, 0, 0.3)' }}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        {coverArtState.file && (
                            <button onClick={handleFitToggle} className="w-full h-12 rounded-xl text-sm font-semibold active:scale-95 transition-all" style={{ backgroundColor: colors.midDark, color: colors.softLight }}>
...
const SystemListSection = memo(({ colors, categories, searchQuery, onSearchChange, currentCore, onSelectSystem, isSearchFocused, setIsSearchFocused }: SystemListSectionProps) => {
    const handleSystemSelect = useCallback((core: string) => () => {
        onSelectSystem(core);
    }, [onSelectSystem]);
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                            {systems.map(([name, core], idx) => {
                                ...
                                return (
                                    <button
                                        key={core}
                                        onClick={handleSystemSelect(core)}
                                        aria-pressed={isSel}
